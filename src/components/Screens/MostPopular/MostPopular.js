...
const MostPopular = React.memo((props) => {
  ...
  const isLoading = useSelector(
    (store) => store.popularStories.mostViewedLoader
  );

  const [storiesLimit, setStoriesLimit] = useState(5);

  const getMostViewed = () => {
    if (mostViewed.length === 0) dispatch(actionStories.mostPopular());
  };
  ...
  if (mostViewed.length > 0) {
    content = mostViewed.map((stories, index) => {
      ...
      if (index < storiesLimit) {
        story = (
          <div key={stories.asset_id} className={classes.contentMainArticles}>
            <div
              style={{
                display: "flex",
    ...
    <div className={classes.layoutContent}>
      ...
      >
        ...
        <p className={classes.contentHeadingText}>
          ...
          <span className={classes.Bold}>NYTimes.com</span> this week...
        </p>
      </div>
      {isLoading ? (
        <Spinner />
