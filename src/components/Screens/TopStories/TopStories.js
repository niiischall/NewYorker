...
const TopStories = React.memo((props) => {
  ...
  const selectedCategory = useSelector(
    (store) => store.topStories.selectedCategory
  );
      ...
      <div className={classes.contentMain} style={{ alignItems: "center" }}>
        <div className={classes.contentMainHeadline}>
          <p className={classes.contentMainHeadlineText}>
            Search for top stories across different sections currently on{" "}
            <span style={{ color: "var(--color-item-sidebar)" }}>
              NYTimes.com
            </span>
          </p>
        </div>
    ...
    content = (
      <div style={{ flex: "1" }}>
        <p className={classes.contentHeadingText}>
          The top stories in{" "}
          <span className={classes.LightBold}>"{selectedCategory.label}"</span>{" "}
          on <span className={classes.Bold}>NYTimes.com</span> currently...
        </p>
        <div
          style={{
            display: "flex",
          ...
          >
            {stories.map((story) => {
              ...
              if (story.multimedia && story.multimedia.length > 0) {
                const multimediaLength = story.multimedia.length;
                imageURL = story.multimedia[multimediaLength - 1].url;
              }

              return (
                <div className={classes.contentMainArticles} key={story.title}>
                  <div className={classes.imageContainer}>
                    <img src={imageURL} alt="STORY" className={classes.image} />
                  </div>
                  <div className={classes.contentContainer}>
                    <span className={classes.title}>{story.title}</span>
                    <span className={classes.ByLine}>{story.byline}</span>
